...
function transactionCommand(
  ...
  if (
    /^(?:SAVEPOINT\b|RELEASE\s+SAVEPOINT\b|ROLLBACK\s+TO\b)/i.test(normalized)
  ) {
    return "in-transaction";
  }
  if (
    /^(?:BEGIN|START\s+TRANSACTION|COMMIT|END|ROLLBACK)\b/i.test(normalized)
  ) {
    throw new Error(
        ...
        async (
          ...
          try {
            const contextualClient =
              currentContext() || requestContextResolver
...
export const pool = patchPoolForRlsContext(
  globalThis.pgPool ||
    new Pool({
