...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    @Query("SELECT p FROM Payment p WHERE p.tenantId = :tenantId AND p.paymentDate BETWEEN :start AND :end ORDER BY p.paymentDate DESC")
    List<Payment> findByTenantIdAndPaymentDateBetween(UUID tenantId, Instant start, Instant end);

    @Query("SELECT SUM(p.amount) FROM Payment p WHERE p.tenantId = :tenantId AND p.paymentDate BETWEEN :start AND :end")
