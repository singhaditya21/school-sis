...
describe('strict rate-limit entrypoint policy', () => {
    ...
    ])('rate limits remaining AI ingress before provider forwarding: %s', (file) => {
        const contents = source(file);
    ...
    it('surfaces rate-limit degradation through readiness and metrics', () => {
        ...
        expect(readiness).toContain("rateLimit.status === 'degraded'");
    ...
    it('ships dashboard and alert definitions for every rate-limit failure signal', () => {
        const alerts = repositorySource('ops/observability/prometheus/rate-limit-alerts.yml');
        const dashboard = JSON.parse(repositorySource('ops/observability/grafana/rate-limit-dashboard.json'));
    ...
    it('consumes the login attempt before authentication without a second failure mutation', () => {
        ...
        expect(contents.indexOf('consumeLoginRateLimit(email)')).toBeLessThan(contents.indexOf('compare(password'));
