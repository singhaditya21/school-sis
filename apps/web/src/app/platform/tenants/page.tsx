    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Tenant Schools</h1>
                    <p className="text-muted-foreground mt-1">Manage all registered school instances on the platform.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {tenants.map(tenant => (
                    <div key={tenant.id} className="bg-white rounded-2xl border border-border shadow-sm p-6 hover:shadow-md transition-shadow">
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-5">
                            <div className="bg-muted rounded-xl p-3 text-center">
                                <p className="text-2xl font-bold text-foreground">{tenant.activeStudents.toLocaleString()}</p>
                                <p className="text-xs text-muted-foreground">Students</p>
                            </div>
                            <div className="bg-muted rounded-xl p-3 text-center">
                                <p className="text-2xl font-bold text-foreground">${tenant.revenue.toLocaleString()}</p>
                        ...
                        <div className="flex gap-2">
                            <form className="flex-1" action={async () => {
                                ...
                                <button className="w-full px-4 py-2.5 bg-muted text-foreground rounded-xl text-sm font-medium hover:bg-slate-200 transition">
                                    View Dashboard
                                </button>
                            </form>
