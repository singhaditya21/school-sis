...
export default async function AIAnalyticsPage() {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
            <div>
                <h1 className="text-3xl font-extrabold text-foreground tracking-tight">AI Economics</h1>
                <p className="text-muted-foreground mt-1">Real-time telemetry of API burn rates versus computed platform inference.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-border shadow-sm relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Total Queries</p>
                        <span className="text-4xl font-bold text-foreground">{hasData ? data.totalQueries.toLocaleString() : '0'}</span>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-border shadow-sm">
                    <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Tokens Processed</p>
                    <span className="text-4xl font-bold text-foreground">{hasData ? (data.totalTokens / 1000).toFixed(1) : '0'}k</span>
                </div>
            ...
            <div>
                <h2 className="text-2xl font-bold text-foreground mb-5">Agent Workload Distribution</h2>
                {!hasData ? (
                    <div className="text-center py-12 bg-white rounded-2xl border border-border shadow-sm">
                        <p className="text-muted-foreground italic">No telemetry logs tracked yet across the company matrix.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {Object.entries(data.agentStats).map(([agentName, stats]) => (
                            <div key={agentName} className="bg-white rounded-2xl border border-border shadow-sm p-6 hover:border-indigo-200 transition">
                                <h3 className="font-bold text-foreground mb-4">{agentName}</h3>
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                                        <span className="text-muted-foreground">Node Invocations</span>
                                        <span className="font-bold text-indigo-700">{stats.queries.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-muted-foreground">Network Drain</span>
