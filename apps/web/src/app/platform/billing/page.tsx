...
export default async function StripeBillingPage() {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
            <div>
                <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Platform Billing</h1>
                <p className="text-muted-foreground mt-1">Manage subscriptions, platform revenue, and tier assignments across all entities.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-border shadow-sm relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Platform MRR (Est)</p>
                        <span className="text-4xl font-bold text-foreground">${totalMRR.toLocaleString()}</span>
                    </div>
                </div>
            ...
            {/* Invoice Table */}
            <div>
                <h2 className="text-2xl font-bold text-foreground mb-5">Recent Invoices</h2>
                <div className="bg-white border border-border rounded-2xl shadow-sm overflow-hidden">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-muted border-b border-border">
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase">Invoice</th>
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase">School</th>
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase">Tier</th>
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase text-right">Amount</th>
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase">Date</th>
                                <th className="p-5 text-sm font-semibold text-muted-foreground uppercase text-center">Status</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {invoices.map(inv => (
                                <tr key={inv.id} className="hover:bg-muted transition-colors">
                                    ...
                                    <td className="p-5 font-medium text-foreground">{inv.school}</td>
                                    <td className="p-5">
                                    ...
                                    <td className="p-5 text-right font-medium text-foreground">${inv.amount.toLocaleString()}</td>
                                    <td className="p-5 text-muted-foreground">{inv.date}</td>
