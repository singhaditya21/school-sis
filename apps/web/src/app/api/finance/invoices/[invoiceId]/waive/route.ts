...
import {
    ...
    waiveInvoiceBalanceWithApproval,
...
export async function POST(request: Request, { params }: RouteContext) {
    ...
    const parsed = waiveSchema.safeParse(json.data);
    if (!parsed.success) {
        return NextResponse.json(
            { error: parsed.error.errors[0]?.message || 'Invalid invoice waiver request.' },
            { status: 400 },
        );
    }
    ...
    try {
        const result = await waiveInvoiceBalanceWithApproval({
        ...
        return financeErrorResponse(error, 'Failed to waive invoice.');
    }
}
