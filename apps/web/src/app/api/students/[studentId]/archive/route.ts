...
export async function POST(request: Request, { params }: RouteContext) {
    ...
    if (!parsed.success) {
        return NextResponse.json(
            { error: parsed.error.errors[0]?.message || 'Invalid student archive request.' },
            { status: 400 },
        );
    }
    ...
    try {
        const result = await archiveStudentWithApproval({
        ...
        return workflowAdoptionErrorResponse(error, 'Failed to archive student.');
    }
}
