...
export default function CreateBatchForm() {
  ...
  async function formAction(formData: FormData) {
    ...
    if (result.success) {
      setMessage({
        type: "success",
        text: `Batch "${result.data?.name}" created successfully!`,
      });
      (
        document.getElementById("coaching-batch-form") as HTMLFormElement
      ).reset();
    } else {
    ...
    <div className="bg-card p-8 rounded-2xl shadow-sm border border-border mt-8">
      ...
      {message && (
        <div
          role="status"
          aria-live="polite"
          className={`p-4 mb-6 rounded-lg border ${message.type === "success" ? "bg-success-subtle text-success-subtle-foreground border-success/20" : "bg-destructive-subtle text-destructive-subtle-foreground border-destructive/20"}`}
        >
          {message.text}
        </div>
      )}
