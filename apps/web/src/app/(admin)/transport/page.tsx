        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">{routes.length} routes configured</p>
                </div>
                    ...
                    routes.map((route) => (
                        <div key={route.id} className="bg-white rounded-xl shadow-sm border p-6">
                            ...
                            <div className="space-y-2 text-sm">
                                <p><span className="text-muted-foreground">Stops:</span> {route.stopCount}</p>
                                <p><span className="text-muted-foreground">Students:</span> {route.studentCount}</p>
                                {route.morningDepartureTime && (
                                    <p><span className="text-muted-foreground">Morning:</span> {route.morningDepartureTime}</p>
                                )}
                                {route.monthlyFee && (
                                    <p><span className="text-muted-foreground">Monthly Fee:</span> {formatCurrency(Number(route.monthlyFee))}</p>
                                )}
                            </div>
