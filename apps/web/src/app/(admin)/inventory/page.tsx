...
import AddItemForms from './add-item-forms';

/** Mirrors the `asset_condition` enum in the database — anything else fails the insert. */
                                        ...
                                        {canWrite && (
                                            <td className="px-4 py-3">
                                                <form action={updateAssetConditionForm} className="flex items-center gap-2">
                                                    ...
                                                    <label className="sr-only" htmlFor={`condition-${a.id}`}>Condition for {a.name}</label>
                                                    <select
                                                        id={`condition-${a.id}`}
                                                        name="condition"
                                                    ...
                                                    >
                                                        {!(ASSET_CONDITIONS as readonly string[]).includes(a.condition) && (
                                        ...
                                        {canWrite && (
                                            <td className="px-4 py-3">
                                                <form action={restockConsumableForm} className="flex items-center gap-2">
                                                    ...
                                                    <label className="sr-only" htmlFor={`restock-${c.id}`}>Restock quantity for {c.name}</label>
                                                    <input
                                                        id={`restock-${c.id}`}
                                                        type="number"
                                                        name="quantity"
