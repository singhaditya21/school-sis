...
export async function getStudentVisasAction() {
    const { tenantId } = await requireCapability('international', 'students:read');
...
export async function createStudentVisaAction(data: {
    ...
    const { tenantId } = await requireCapability('international', 'students:update');
    ...
    `, [tenantId, data.studentId, data.visaType, data.countryOfOrigin, data.passportNumber, data.issueDate, data.expirationDate]);
...
export async function createHostFamilyAction(data: { familyName: string; address: string; phone: string; backgroundChecked?: string }) {
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
    `, [tenantId, data.familyName, data.address, data.phone, data.backgroundChecked ? new Date(data.backgroundChecked) : null]);
...
export async function createInternationalPlacementAction(data: { studentId: string; hostFamilyId?: string; placementYear: string }) {
    ...
    await pool.query(`
        ...
        VALUES ($1, $2, $3, $4)
    `, [tenantId, data.studentId, data.hostFamilyId || null, data.placementYear]);
