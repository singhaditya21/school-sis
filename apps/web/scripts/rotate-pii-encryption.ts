...
const deterministicTarget = (
  ...
  column: string,
  purpose: string,
  encryptValue: (plaintext: string) => string = (plaintext) =>
    encryptDeterministic(plaintext, purpose),
): RotationTarget => ({
  table,
  column,
  ...
  try {
    ...
    for (const row of result.rows) {
      ...
      if (update.rowCount !== 1) {
        throw new Error(
          `Concurrent update detected for ${target.table}.${target.column} id=${row.id}.`,
        );
      }
    }
...
async function main(): Promise<void> {
  ...
  if (keyStatus.legacyFallbackInUse) {
    throw new Error(
      "PII_ENCRYPTION_KEY must be set explicitly before rotating data.",
    );
  }

  console.info(`PII key: ${keyStatus.currentKeyId}`);
  console.info(
    `Previous key configured: ${keyStatus.rotationActive ? "yes" : "no"}`,
  );
  ...
  try {
    ...
    for (const target of TARGETS) {
      ...
      if (remaining !== 0) {
        throw new Error(
          `${target.table}.${target.column} still has ${remaining} values on an older key.`,
        );
      }
    }
    ...
    if (assertCurrent && totalPending > 0) {
      throw new Error(
        `${totalPending} encrypted value(s) are not on the current PII key.`,
      );
    }
    if (!execute && totalPending > 0) {
      console.info(
        `Dry-run complete: ${totalPending} value(s) require rotation. Re-run with --execute.`,
      );
    } else {
      console.info(
        `Rotation verification complete: ${execute ? 0 : totalPending} value(s) remain.`,
      );
    }
  } finally {
    await pool.end();
