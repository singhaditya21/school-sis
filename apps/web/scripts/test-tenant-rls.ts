...
if (!connectionString) {
    console.error('DATABASE_URL or DIRECT_URL is required for the tenant RLS integration test.');
    process.exit(1);
}
...
async function main(): Promise<void> {
    ...
    try {
        ...
        const uncovered = coverage.rows.filter(
            (row) => !row.row_security || !row.force_row_security || Number(row.policy_count) < 1,
        );
        assert(uncovered.length === 0, `RLS coverage gaps: ${JSON.stringify(uncovered)}`);
        ...
        await client.query(`
            ...
            $block$;
        `);
        await client.query(`GRANT USAGE ON SCHEMA public, app_private TO ${TEST_ROLE}`);
        await client.query(`
        ...
        const visibleRows = await client.query<{ id: string }>('SELECT id FROM academic_years ORDER BY id');
        assert(visibleRows.rows.length === 1 && visibleRows.rows[0].id === YEAR_A, 'Tenant A must see only its own row.');
        ...
        assert(crossTenantUpdate.rowCount === 0, 'Cross-tenant updates must affect zero rows.');

        const crossTenantDelete = await client.query('DELETE FROM academic_years WHERE id = $1', [YEAR_B]);
        assert(crossTenantDelete.rowCount === 0, 'Cross-tenant deletes must affect zero rows.');
        ...
        assert(rejectedCrossTenantInsert, 'Cross-tenant inserts must be rejected by RLS.');
        ...
        try {
            ...
            rejectedForeignObjectRecord = (error as { code?: string }).code === '42501';
            await client.query('ROLLBACK TO SAVEPOINT foreign_metadata_object');
            await client.query('RELEASE SAVEPOINT foreign_metadata_object');
        }
        assert(rejectedForeignObjectRecord, 'A tenant record must not reference another tenant metadata object.');
        ...
        assert(rejectedCrossObjectField, 'A metadata value field must belong to the record metadata object.');
        ...
        assert(noContextRows.rowCount === 0, 'A non-platform session without tenant context must see zero rows.');

        await client.query('RESET ROLE');
        await client.query('ROLLBACK');
        console.info(`Tenant RLS integration test passed across ${coverage.rows.length} protected tables.`);
    } catch (error) {
        try {
            await client.query('RESET ROLE');
            await client.query('ROLLBACK');
