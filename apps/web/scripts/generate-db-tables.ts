...
async function main(): Promise<void> {
    ...
    try {
        ...
        for (const { table_name } of tableRows) {
            ...
            // Column-reference object (qualified identifiers) + snake-case name map
            parts.push(`export const ${camel} = {`);
            parts.push(`    $name: "${table_name}",`);
            for (const c of columns) {
                parts.push(`    ${toCamel(c.column_name)}: identifier("${table_name}", "${c.column_name}"),`);
            }
            parts.push(`} satisfies { $name: string } & Record<string, SqlQuery | string>;`);
            parts.push(`export const ${camel}Columns = {`);
            for (const c of columns) {
                parts.push(`    ${toCamel(c.column_name)}: "${c.column_name}",`);
            }
            parts.push(`} as const;`, "");
        }
