...
export default async function globalSetup() {
    ...
    const originalDbName = urlObj.pathname.slice(1);
    const testDbName = 'school_sis_test';
    ...
    // We'll write this to a .env.test file so the web server can pick it up
    const envTestPath = path.resolve(__dirname, '../.env.test');
    fs.writeFileSync(envTestPath, `DATABASE_URL="${testDbUrl}"\n`);
    console.log(`📝 Wrote test environment variables to .env.test`);
